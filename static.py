...
def static_web(fname, base_url,list_content,pages): #list_content list of all types in urls to get ['articles','discussions','speeches','pearls']
   #pages=[13,10,21,9]#[317,820,380,120]
   #for j,sub in enumerate(list_content):#='fatwas/kind/'#
   #for j in range(3,5):#5
   j,sub=0,list_content
   tld = base_url+sub#+str(j)
   for i in range(1,pages+1):
    write_data(tld+'?page=' + str(i),fname+'.csv',sub,'kind '+str(j),'page '+str(i))#fatwa_flag=True
